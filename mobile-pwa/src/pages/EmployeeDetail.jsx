...
const generateMobileWhatsAppMessage = (data) => {
  const { employee, attendance, salary, period, absentDates, sundayWork } = data;
  
  let message = `*📋 Attendance & Salary Report*
...
🚫 Unpaid Leaves: ${attendance.unpaid_leaves}`;
  ...
  // Add Sunday work details if any
  if (sundayWork && (sundayWork.compensation > 0 || sundayWork.overtime > 0)) {
    message += `\n\n*🌅 Sunday Work:*`;
    if (sundayWork.compensation > 0) {
      message += `\n✅ Compensation Days: ${sundayWork.compensation} (compensated ${sundayWork.compensation} absence${sundayWork.compensation > 1 ? 's' : ''})`;
    }
    if (sundayWork.overtime > 0) {
      message += `\n💰 Overtime Days: ${sundayWork.overtime} (₹${sundayWork.overtime_pay.toFixed(2)})`;
    }
  }

  // Add absent dates if any
  if (absentDates && absentDates.length > 0) {
    message += `\n\n*Absent Dates:*`;
    absentDates.forEach(record => {
      const date = format(parseISO(record.attendance_date), 'dd MMM yyyy');
      const type = record.is_paid_leave ? '💼 Paid' : '🚫 Unpaid';
      const reason = record.absence_reason ? ` - ${record.absence_reason}` : '';
      message += `\n${date} (${type})${reason}`;
    });
  }
...
Per Day Rate: ₹${salary.per_day_salary.toFixed(2)}`;
  ...
  if (sundayWork && sundayWork.compensation > 0) {
    message += `\nSunday Compensation: ${sundayWork.compensation} day${sundayWork.compensation > 1 ? 's' : ''} (saved ₹${(salary.per_day_salary * sundayWork.compensation).toFixed(2)})`;
  }
  
  message += `\nDeduction (${attendance.unpaid_leaves} unpaid): -₹${salary.deduction.toFixed(2)}`;
  
  if (sundayWork && sundayWork.overtime > 0) {
    message += `\nOvertime (${sundayWork.overtime} Sunday${sundayWork.overtime > 1 ? 's' : ''}): +₹${sundayWork.overtime_pay.toFixed(2)}`;
  }
...
export default function EmployeeDetail() {
  ...
  const generateAbsenteeReport = async () => {
    try {
      ...
      const presentDays = attendance.filter(a => a.is_present && !a.is_sunday_work).length;
      ...
      // Calculate Sunday work
      const sundayWorkDays = attendance.filter(a => a.is_present && a.is_sunday_work).length;
      const sundayCompensationDays = Math.min(sundayWorkDays, unpaidLeaveDays);
      const sundayOvertimeDays = sundayWorkDays - sundayCompensationDays;
      const actualUnpaidAbsences = unpaidLeaveDays - sundayCompensationDays;

      const perDaySalary = employee.monthly_salary / workingDays;
      const deduction = actualUnpaidAbsences * perDaySalary;
      const overtimePay = sundayOvertimeDays * perDaySalary;
      const finalSalary = employee.monthly_salary - deduction + overtimePay;
      ...
      const message = generateMobileWhatsAppMessage({
        ...
        sundayWork: {
          compensation: sundayCompensationDays,
          overtime: sundayOvertimeDays,
